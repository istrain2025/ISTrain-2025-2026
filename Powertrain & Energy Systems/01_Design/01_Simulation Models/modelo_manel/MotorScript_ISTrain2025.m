...
switch bt
    case 1
        ...
        voc_cell = [2.50  3.00  3.20  3.26  3.32  3.35  3.40];
...
%% 3) Drive‑Cycle Definition
    ...
    case 6  % 0 -> 15 km/h with constant acceleration + CRUISE
    % Parameters
    ...
    t_accel = v_final / a;       % [s] (Aprox. 10.4s)
    
    % --- TEMPO EXTRA ADICIONADO AQUI ---
    t_cruise = 500;              % [s] Tempo que queres que ele ande a velocidade constante (Altera este valor à vontade!)
    duration_s = t_accel + t_cruise;
    % -----------------------------------
    ...
    speed = (a * time) * 3.6;     % [km/h]
    speed = min(speed, v_final_kmh);  % ensure it doesn't exceed 15 km/h (corta a rampa aos 15km/h e mantém)
    ...
    assignin('base', 'grade', 0.02);          % [-]
    assignin('base', 'friction_coeff', 0.004);
    assignin('base', 'm_trail', 2400);
    stopTime = time(end);

    case 7  % 0 -> 15 km/h -> 1 min cruise -> 0 km/h (Using Phase Concatenation)
        % Parameters
        a_acc      = 0.4;                     % [m/s^2] acceleration
        a_dec      = 0.4;                     % [m/s^2] deceleration (magnitude)
        v_max      = 15 * 1000/3600;          % [m/s] max speed
        t_cruise   = 21600;                      % [s] cruise hold time
        
        % 1) First phase: Acceleration
        t_acc = v_max / a_acc;
        % Sampling at 10 Hz (ceil(t*10))
        time1 = linspace(0, t_acc, ceil(t_acc * 10))';
        v1_vec = (a_acc * time1) * 3.6;       % [km/h]
        
        % 2) Second phase: Cruise (Constant Speed)
        t_start_cruise = time1(end) + 0.1;
        t_end_cruise   = t_start_cruise + t_cruise;
        time2 = linspace(t_start_cruise, t_end_cruise, t_cruise * 10)';
        v2_vec = repmat(15, numel(time2), 1); % [km/h] holds at 15
        
        % 3) Third phase: Deceleration
        t_dec = v_max / a_dec;
        t_start_dec = time2(end) + 0.1;
        t_end_dec   = t_start_dec + t_dec;
        time3 = linspace(t_start_dec, t_end_dec, ceil(t_dec * 10))';
        
        % Local time for decel starting at 0.1s for accurate math
        t_local_dec = time3 - time3(1) + 0.1;
        v3_vec = (v_max - (a_dec * t_local_dec)) * 3.6; % [km/h]
        v3_vec = max(v3_vec, 0);              % Floor to 0 to prevent negative speeds
        
        % 4) Concatenate all phases
        time  = [time1; time2; time3];
        speed = [v1_vec; v2_vec; v3_vec];
        ...
        % Assign physical load and track conditions
        assignin('base', 'grade', 0.000001);          % [%]
        assignin('base', 'friction_coeff', 0.004);
        assignin('base', 'm_trail', 2400);        % [kg]
