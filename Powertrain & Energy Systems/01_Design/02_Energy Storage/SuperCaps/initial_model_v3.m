%% -------------------- Vehicle / track (ISTrain) ------------------------
m_loco   = 600 + 1800;   % [kg] loco + trailing mass
g        = 9.81;         % [m/s^2]
Crr      = 0.004;        % [-] rolling resistance coeff
rw       = 0.100;        % [m] wheel radius
G        = 10;           % [-] gearbox ratio
mu       = 0.35;         % [-] adhesion (wheel–rail)
...
Vmin_cap   = 44.0;       % [V] minimum usable cap voltage on discharge (converter floor)

eta_tx     = 0.85;       % transmission/drivetrain efficiency
eta_motor  = 0.90;       % motor+inverter efficiency
eta_dc     = 0.85;       % DC/DC converter efficiency
eta_sc     = 0.90;       % supercap charge efficiency

% Directional paths (same definitions as before)
...
%% -------------------- Supercapacitor (ISTrain) -------------------------
% keep the 48 V, 165 F module unless you want to switch
C   = 165;               % [F]
ESR = 0.0063;            % [ohm]
cap_name = '48V 165F';
...
for k = 1:N-1
    ...
    FregenA(k)  = F_used;
    TperMA(k)   = 1.3*(F_used*rw)/(G*eta_tx)/n_m;
...
TperMB = zeros(NB,1);   % torque per motor during Phase B (motoring)
PwheelsB = zeros(NB,1);
IbusB    = zeros(NB,1);
aB       = zeros(NB,1);
FmotB   = zeros(NB,1);   % motor force at wheels during Phase B (tractive)
...
for k = 1:NB-1
    ...
    % --- Torque per motor (motoring) ----------------------------------------
    if vB(k) > 1e-3
        F_mot_B = F_rr + PwheelsB(k) / vB(k) + (600+1800) * g * 0.02;     % wheel force delivered by motors
    else
        F_mot_B = F_rr + m_loco * aB(k);          % v≈0 fallback (P/v ill-conditioned)
    end
    FmotB(k) = F_mot_B;
    TperMB(k) = 1.3*(F_mot_B * rw) / (G * eta_tx) / n_m;   % [Nm] per motor
...
% --- Braking forces (Phase A) + Motor force (Phase B) ---
subplot(2,3,5);
plot(tA, FregenA, 'LineWidth', 1.8); hold on; grid on;
yline(F_rr,'--','F_{rr}');
yline(F_adh,':','F_{adh}');
% NEW: Motor tractive force during Phase B (offset in time)
plot(tB(1:end-1) + tA(end), FmotB, 'm', 'LineWidth', 1.8);

xlabel('Time [s]'); ylabel('Force [N]');
title('Forças: regen (A), tração (B), rolamento e aderência');
legend('F_{regen} (A)','F_{rr}','F_{adh}','F_{motor} (B)','Location','best');
...
figure('Name','ISTrain Comparison: Power','Color','w'); hold on; grid on;
...
%% Torque per motor over time (regen + motoring)
figure('Name','Torque per Motor','Color','w'); hold on; grid on;

% Regen torque (negative sign for braking direction)
plot(tA(1:end-1), TperMA(1:end-1), 'b', 'LineWidth', 1.8);

% Motoring torque
plot(tB(1:end-1) + tA(end), TperMB, 'r', 'LineWidth', 1.8);

% Adhesion-limit torque per motor (based on your F_adh definition)
T_adh_lim = (F_adh * rw) / (G * eta_tx) / n_m;
yline(T_adh_lim, '--', 'T_{adh,lim}', 'LineWidth', 1.2);

xlabel('Time [s]');
ylabel('Torque per motor [Nm]');
title('Torque per Motor: braking (−) and motoring (+)');
legend('Regen torque','Motoring torque','Adhesion-limit torque','Location','best');
